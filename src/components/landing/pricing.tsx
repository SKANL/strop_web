      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl sm:text-center">
          ...
          <p className="mt-6 text-lg leading-8 text-gray-600 dark:text-gray-300">
            Paga una fracción de lo que te ahorramos en mermas y desvíos desde el primer mes.
          </p>
        </div>
        <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 gap-8 lg:max-w-none lg:grid-cols-3">
          {tiers.map((tier) => (
             <div key={tier.id} className={`flex flex-col justify-between rounded-3xl p-8 ring-1 xl:p-10 ${tier.mostPopular ? 'bg-zinc-900 ring-zinc-900 lg:z-10 lg:scale-105 shadow-xl' : 'bg-white ring-gray-200 dark:bg-zinc-900 dark:ring-gray-700'}`}>
               <div>
                   ...
                   <p className="mt-6 flex items-baseline gap-x-1">
                     <span className={`text-4xl font-bold tracking-tight ${tier.mostPopular ? 'text-white' : 'text-gray-900 dark:text-white'}`}>{tier.priceMonthly}</span>
                     {tier.priceMonthly !== 'Personalizado' && <span className={`text-sm font-semibold leading-6 ${tier.mostPopular ? 'text-gray-300' : 'text-gray-600 dark:text-gray-400'}`}>/mes</span>}
                   </p>
               ...
               <Button className={`mt-8 block w-full rounded-md px-3 py-2 text-center text-sm font-semibold leading-6 focus-visible:outline-2 focus-visible:outline-offset-2 ${tier.mostPopular ? 'bg-orange-600 hover:bg-orange-500 text-white focus-visible:outline-orange-600' : 'bg-orange-50 text-orange-600 hover:bg-orange-100 dark:bg-zinc-800 dark:text-white dark:hover:bg-zinc-700'}`} variant={tier.mostPopular ? 'default' : 'secondary'}>
                  Elegir plan
               </Button>
             </div>
          ))}
